...
public class LunarCalendar {
    ...
    private static final Set<Integer> LEAP_MOUTH = ImmutableSet.of(184904, 185108, 185407, 185705, 186003, 186208,
        186505, 186804, 187010, 187306, 187605, 187903, 188107, 188405, 188704, 189002, 189206, 189505, 189803,
        190008, 190305, 190604, 190902, 191106, 191405, 191702, 191907, 192205, 192504, 192802, 193006, 193305,
        193603, 193807, 194106, 194404, 194702, 194907, 195205, 195503, 195708, 196006, 196304, 196603, 196807,
        197105, 197404, 197608, 197906, 198204, 198410, 198706, 199005, 199303, 199508, 199805, 200104, 200402,
        200607, 200905, 201204, 201409, 201706, 202004, 202302, 202506, 202805, 203103, 203311, 203606, 203905,
        204202, 204407, 204705, 205003, 205208, 205506, 205804, 206103, 206307, 206605, 206904, 207108, 207406,
        207704, 208003, 208207, 208505, 208804, 209008, 209306, 209604, 209902, 210107, 210405, 210704, 210909,
        211206, 211504, 211803);
    /**
    ...
    private static boolean isLeapMonth(int year, int mouth) {
        return LEAP_MOUTH.contains(year * 10 + mouth);
    }
     ...
     * @param monthDay 农历日
     */
    public static Calendar lunarToSolar(int year, int month, int monthDay) {
        int dayOffset;
        int leapMonth;
        int i;
        ...
        int theLunarOffset = LUNAR_INFO[year - MIN_YEAR];
        dayOffset = (theLunarOffset & 0x001F) - 1;

        if (((theLunarOffset & 0x0060) >> 5) == 2) { dayOffset += 31; }

        for (i = 1; i < month; i++) {
            if ((theLunarOffset & (0x80000 >> (i - 1))) == 0) { dayOffset += 29; } else { dayOffset += 30; }
        }

        dayOffset += monthDay;
        leapMonth = (theLunarOffset & 0xf00000) >> 20;

        // 这一年有闰月
        if (leapMonth != 0) {
            if (month > leapMonth || (month == leapMonth && isLeapMonth)) {
                if ((theLunarOffset & (0x80000 >> (month - 1))) == 0) { dayOffset += 29; } else { dayOffset += 30; }
            }
        }
        ...
        return new GregorianCalendar(solarInfo[0], solarInfo[1] - 1, solarInfo[2]);
    }
